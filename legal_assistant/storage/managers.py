...
FIXED: All import issues and missing variables resolved.
"""
import asyncio
...
class EnhancedStorageManager:
    ...
    async def initialize(self):
        """Initialize NoSQL connections with graceful error handling"""
        if not self._initialized:
            try:
                # Try to initialize NoSQL - if this fails, we continue with in-memory
                ...
                logger.info("✅ Enhanced storage initialized with NoSQL support")
    ...
    async def _migrate_existing_data(self):
        ...
        try:
            # Only attempt migration if we have data to migrate
    ...
    async def set_processing_status(self, file_id: str, status: str, progress: int = 0,
        ...
        if self.nosql_manager and getattr(self.nosql_manager, 'mongodb_available', False):
            try:
                from .nosql_models import ProcessingStatusDocument
    ...
    async def add_to_conversation(self, session_id: str, role: str, content: str, sources: Optional[List] = None):
        ...
        if self.nosql_manager and getattr(self.nosql_manager, 'mongodb_available', False):
            try:
                from .nosql_models import ConversationDocument
    ...
    async def cache_set(self, key: str, value: Any, ttl: int = 3600):
        ...
        if self.nosql_manager and getattr(self.nosql_manager, 'redis_available', False):
            try:
                import json
    ...
    async def cleanup_old_data(self):
        ...
        if self.nosql_manager and getattr(self.nosql_manager, 'mongodb_available', False):
            try:
                from .nosql_models import ConversationDocument, ProcessingStatusDocument
...
# FIXED: Immigration storage that was missing
immigration_cases: Dict[str, Any] = {}
case_documents: Dict[str, List[str]] = {}  # ← This was missing and causing the import error!
deadline_alerts: Dict[str, Any] = {}
resource_library: Dict[str, Any] = {}

# Initialize resource library with default immigration resources
resource_library.update({
    "asylum_guide_en": {
        "resource_id": "asylum_guide_en",
        "title": "Asylum Application Guide",
        "category": "guide", 
        "languages": ["en"],
        "content": "Guide for preparing asylum applications including required documentation, interview preparation, and evidence gathering strategies.",
        "tags": ["asylum", "guide"],
        "last_updated": datetime.utcnow(),
        "downloads": 0
    },
    "rfe_response_template": {
        "resource_id": "rfe_response_template", 
        "title": "RFE Response Template",
        "category": "template",
        "languages": ["en"],
        "content": "Template for responding to Requests for Evidence including organization strategies and documentation requirements.",
        "tags": ["rfe", "template"],
        "last_updated": datetime.utcnow(),
        "downloads": 0
    },
    "credible_fear_prep": {
        "resource_id": "credible_fear_prep",
        "title": "Credible Fear Interview Preparation",
        "category": "guide",
        "languages": ["en", "es"],
        "content": "Comprehensive guide for preparing clients for credible fear interviews including common questions and testimony strategies.",
        "tags": ["credible_fear", "interview", "preparation"],
        "last_updated": datetime.utcnow(), 
        "downloads": 0
    }
})
...
async def _sync_from_nosql():
    ...
    try:
        ...
        global immigration_cases, case_documents, deadline_alerts, resource_library
        ...
        immigration_cases = storage.immigration_cases
        case_documents = storage.case_documents
        deadline_alerts = storage.deadline_alerts
        
        # Merge with default resource library
        storage.resource_library.update(resource_library)
        resource_library = storage.resource_library
...
def add_to_conversation(session_id: str, role: str, content: str, sources: Optional[List] = None):
    ...
    # Simple approach that works without async complexity
